...
//---------------------------------------Server V0---------------------------------//
bool TX90::Login(const string& url, const string& userName, const string& password){
	if(mLogin)
		delete mLogin;
	if(mLoginResponse)
  ...
  mEndpoint = "http://" + url + "/" ;
	mEndpointV1 = mEndpoint + "CS8ServerV1";
	mEndpointV3 = mEndpoint + "CS8ServerV3";
...
bool TX90::SetJoints(const std::vector<double>& joints){
	if(joints.size() != 6)
		return false;
	if(!mIsLoggedIn)
		return false;
...
bool TX90::GetRobotJoints(std::vector<double>& joints){
	if(!mIsLoggedIn)
		return false;
...
bool TX90::GetRobotCartesianPosition(std::vector<double>& position){
	if(!mIsLoggedIn)
		return false;
...
bool TX90::GetRobots(std::vector<int>& robots){
	if(!mIsLoggedIn)
		return false;
...
bool TX90::Ping(std::string text){
  if(!mIsLoggedIn)
		return false;
  _ns1__ping robReq;
  robReq.message = &text;
  _ns1__pingResponse robRes;
  mCS8ServerV0->ping(&robReq, &robRes);
  return robRes.message->compare("PONG: " + text) == 0 ? true : false;
}

//----------------------------------------Server V1----------------------------------------//
bool TX90::GetApplications(std::vector<std::string>& appNames){
	if(!mIsLoggedIn)
		return false;
...
bool TX90::GetJointRange(std::vector<double>& min, std::vector<double>& max){
	if(!mIsLoggedIn)
		return false;
...
bool TX90::MoveJoints(std::vector<double> jnts){
	if(!mIsLoggedIn)
		return false;
...
bool TX90::InverseKinematics(std::vector<double> pos, std::vector<double> start, std::vector<double> &jnts){
	if(!mIsLoggedIn)
		return false;
...
bool TX90::ForwardKinematics(std::vector<double> jnts, std::vector<double> &pos){
	if(!mIsLoggedIn)
		return false;
...
bool TX90::Power(bool on){
  ...
  if( powerResponse->code == ns6__PowerReturnCode__POWER_NO_ERR )
    powerOn = true;
...
bool TX90::MoveLine(std::vector<double> pos){
	if(!mIsLoggedIn)
		return false;
	...
	if( moveResponse->motRet == ns6__MotionReturnCode__SCSV3_MOT_NO_ERR )
	  resp = true;
	delete moveL;
	delete moveResponse;
	return resp;
...
bool TX90::MoveCart(std::vector<double> pos){
	if(!mIsLoggedIn)
		return false;
...
void TX90::GetRxRyRzCoord(ns6__Frame *x_fr, double *x_Rx, double *x_Ry, double *x_Rz)
{
...
void TX90::SetFrameFromRxRyRz(ns6__Frame *x_fr, double x_Rx, double x_Ry, double x_Rz)
{
...
void TX90::InitializeMotionDesc(ns6__MotionDesc* md){
...
void TX90::InitializeConfig(ns6__Config * conf){
