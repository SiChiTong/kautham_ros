...
namespace libPlanner {
  namespace PRM{
   ...
   //!Update boost graph data
    ...
    bool PRMPlanner::connectSamples(bool assumeAllwaysFree)
	{
	    int n;
	    Sample *smpFrom;
	    Sample *smpTo;
    ...
    void PRMPlanner::printConnectedComponents()
	{
		...
		{
			cout<<"SDK sampler generated "<<((GaussianLikeSampler*)_sampler)->ss->getSize()<<" samples"<<endl;
			int k=0;
			for(int i=0;i<((GaussianLikeSampler*)_sampler)->ss->getSize();i++)
			{
				if((((GaussianLikeSampler*)_sampler)->ss->getSampleAt(i))->getcolor()!=0) k++;
			}
			cout<<"and evaluated "<<k<<endl;
		}
	}
	...
	void PRMPlanner::smoothPath(bool maintainfirst, bool maintainlast)
	{
		if(!_solved){
