                                       ...
                                       kautham_ros::ReqCollisionCheck::Response &res)
{
  ...
  //ROS_ERROR("SrvCollisionCheck launched.");
...
//ROS_ERROR("Before distance check 2 Robots");
  if (_problem->wSpace()->distanceCheck2Robots(smp) < 300)
  //if (_problem->wSpace()->collisionCheck(smp))
  {
    //ROS_ERROR("Collision for distance");
    res.collision = true;
...
bool Kauthamplanner::ArmInverseKinematics(unsigned int Robot, vector<KthReal> &carm)
{
  ...
  {
    cout << "Sample in collision with FIX obstacle" << endl;
    return false;
  }
  else{
    cout << "Sample without collision with a FIX obstacle" << endl;
    return true;
  }
}
